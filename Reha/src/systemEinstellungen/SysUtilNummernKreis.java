...
public class SysUtilNummernKreis extends JXPanel implements KeyListener, ActionListener {
	...
	public SysUtilNummernKreis(ImageIcon img){
		...
		JScrollPane jscr = JCompTools.getTransparentScrollPane((SystemConfig.mitRs ? getVorlagenSeiteMitRs() : getVorlagenSeiteOhneRs()));
		jscr.validate();
		jscr.getVerticalScrollBar().setUnitIncrement(15);
		add(jscr,BorderLayout.CENTER);
		/*
		if(SystemConfig.mitRs){
			add(getVorlagenSeiteMitRs(),BorderLayout.CENTER);
		}else{
			add(getVorlagenSeiteOhneRs(),BorderLayout.CENTER);	
		}
		*/
